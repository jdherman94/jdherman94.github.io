...
#Note: This program contains code recycled from program used in my assignment 4 submission
import numpy as np
import sympy as sym
import pandas as pd
...
def create_BEAM_TITLE(dpsifm_units,problem_title):
    unit_tuples_dict={1:('y displacements d',dpsifm_units[0]),2:('z displacements psi',dpsifm_units[1]),\
                     3:('y forces f',dpsifm_units[2]),4:("z forces m",dpsifm_units[3])}
    problem_type="FEA BEAM ANALYSIS"
...
def FEA_BEAM(E_list,I_list,L_list,q_sym_list,M,fdpairs,title="FEA BEAM ANALYSIS"):
    """
    ...
    L_list is list of cross sectional areas for each element input in manner as described
        above
    ...
    fdpairs is dictionary containing known values of forces/displacements for each
        ...
        {1:[[4,'f'],[0,'d']]} means 1st global node has its global y force(zeroth index) fixed at force fixed at 4,
        while global z displacement (first index) (angular) displacement fixed at 0. Dictionary element force or displacement must
        be specified for all global nodes with no nodes referenced twice.  """
    #Inputs dictionary
    inputs={"E_list":E_list,"I_list":I_list,"L_list":L_list,\
    ...
    L,keval_beam,xbar=sym.symbols('L,keval_beam,xbar')




    keval_beam_list=E_list*I_list#Element stiffness calculation
    ...
    Ke=1/L**3*sym.Matrix([[12,6*L,-12,6*L],
                        [6*L,4*L**2,-6*L,2*L**2],
                        [-12,-6*L,12,-6*L],
                        [6*L,2*L**2,-6*L,4*L**2]])
    ...
    Kelist=[np.array(Ke.subs({L:L_list[e-1],keval_beam:keval_beam_list[e-1]})).astype(np.float) for e in range(1,N+1)]
    #add to dictionary
    ...
    def equivalent_nodal_forces(q):
        if q!=0:
            ...
            f0_hat_e=-L*sym.integrate(q.subs({x_hat:L*xbar})*N.T,(xbar,0,1))#Equivalent Nodal Forces
        else:
            f0_hat_e=sym.zeros(4,1)
        return f0_hat_e

    #CALCULATE EQUIVALENT NODAL FORCES FOR EACH ELEMENT.THEN CONVERT TO GLOBAL COORDINATES

    f_0_e_list=[np.array(equivalent_nodal_forces(q_sym_list[i]).subs({L:L_list[i]})).astype(np.float)  for i in range(N)]
    #add to dictionary
    ...
    d_e_list=[np.array([[d[B[e,1-1]-1,0],d[B[e,2-1]-1,0],d[B[e,3-1]-1,0],d[B[e,4-1]-1,0]]]).T for e in range(N)]
    #add to dictionary
    ...
    for e in range(N):
        print("Below are global/local coordinate forces and displacements for Element %s:"%(e+1))
        print("f_%s_y%s = %s"%(e+1,1,np.format_float_scientific(f_e_list[e][1-1,0],precision=4)))
        print("m_%s_z%s = %s"%(e+1,1,np.format_float_scientific(f_e_list[e][2-1,0],precision=4)))
        print("f_%s_y%s = %s"%(e+1,2,np.format_float_scientific(f_e_list[e][3-1,0],precision=4)))
        print("m_%s_z%s = %s"%(e+1,2,np.format_float_scientific(f_e_list[e][4-1,0],precision=4)))
        print("\n")
        print("d_%s_y%s = %s"%(e+1,1,np.format_float_scientific(d_e_list[e][1-1,0],precision=4)))
        print("phi_%s_z%s = %s"%(e+1,1,np.format_float_scientific(d_e_list[e][2-1,0],precision=4)))
        print("d_%s_y%s = %s"%(e+1,2,np.format_float_scientific(d_e_list[e][3-1,0],precision=4)))
        print("phi_%s_z%s = %s"%(e+1,2,np.format_float_scientific(d_e_list[e][4-1,0],precision=4)))
        print('\n')
